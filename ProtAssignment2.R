ProtAssignment2 <- function(proteinfull, proteinnorm, multExpanded1_withDE, phosphonorm){
...
system.time(for(i in seq_along(multExpanded1_withDE[,1])){
  ...
  if(nrow(matchingGroups) != 0){
    if(nrow(matchingGroups) > 1){#which has the most counts?
      ...
      tmp <- assignedProtein[c("Majority.protein.IDs","HL18862", "HL18486", "HL19160")]
      protein_norm <- rbind(protein_norm,tmp)
    }else{
      tmp <- matchingGroups[c("Majority.protein.IDs","HL18862", "HL18486", "HL19160")]
      protein_norm <- rbind(protein_norm,tmp)
    }
  }
  if(dim(protein_norm)[1]==1){#set names of dataframe in the event the first loop doesn't produce a match
...
#trying to make this more efficient. use a list instead of a data frame

# protein_norm <- vector(mode = "list", length = nrow(multExpanded1_withDE) ) #create an empty list
# 
# 
# system.time(for(i in seq_along(protein_norm)){
#   
#   #get the peptide to search
#   peptide <- as.character(multExpanded1_withDE$Phospho..STY..Probabilities[i])
#   peptide <- gsub(pattern = " *\\(.*?\\) *", replacement = "", peptide)
...
#       counts <- matchingGroups$Razor...unique.peptides
#       assignedProtein <- matchingGroups[which.max(counts),] #need to fix this to return desired numbers
#       protein_norm[i] <- assignedProtein[c("Majority.protein.IDs","HL18862", "HL18486", "HL19160")]
#     }
#     protein_norm[i] <- matchingGroups[c("Majority.protein.IDs","HL18862", "HL18486", "HL19160")]
#   }else{
#     protein_norm[i] <- rep(NA,4)
#   }
# }
# )
# test <- do.call("rbind", protein_norm)
# 
# 
# 
# 
...
# protein_norm <- data.frame()
# 
# system.time(foreach(i=1:nrow(multExpanded1_withDE), .packages = "qdapRegex") %dopar% {
#   #get the peptide to search
#   peptide <- as.character(multExpanded1_withDE$Phospho..STY..Probabilities[i])
#   peptide <- gsub(pattern = " *\\(.*?\\) *", replacement = "", peptide)
#   
...
# stopCluster(cl)
# 
# #unsure how race conditions may affect the result here...
