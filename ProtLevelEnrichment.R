...
##s-curve of ibaq levels with legend as well as density scatter with regression line + pvalue overlay
...
x <- -log10(as.numeric(multExpanded1_withDE_annotated$GelPrepNormFAdjPval))
y <- log10(multExpanded1_withDE_annotated$ppiBAQ.18862)
plot(x,y)
n <- min(sum(complete.cases(x)), sum(complete.cases(y)))
R <- cor(x,y, use = "complete.obs")
p.value <- 2*pt(-abs(R * sqrt(n-2) / sqrt(1-R*R)),df=n-2)
cor.test(x,y)$p.value ##pvalues are the same

#linear regression line
reg.line <- lm(y~x, na.action = "na.omit")
abline(reg.line)

##reactome and GO enrichments followed by wordcloud depiction of enrichment





##barplot for connectivity, % Disorder, extent of PTM modifications, and 
